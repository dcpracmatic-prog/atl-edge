"""
Descubrimiento estructural validado (cross-val + delta-AUC fuera de muestra).

Origen: `structural_discovery_toolkit` (trabajo previo de Arturo), integrado
aquí como el motor real detrás de la huella de gobernanza del Smart Token —
reemplaza la transformada de Walsh-Hadamard determinista (ver
`smart_token.structural`) cuando la fuente es datos de uso reales con ruido,
en vez de una tabla de política fija.

No afirma causalidad. La estabilidad se mide por folds; la utilidad
predictiva se valida fuera de muestra.
"""
...
class StructuralReport:
    ...
    def summary(self, top_k=15):
        print("="*72)
        print("STRUCTURAL DISCOVERY REPORT")
        print("="*72)
...
def discover_structure(
    X: np.ndarray, y: np.ndarray, flag_names: Optional[list]=None,
    max_degree: int=4, folds: int=5, min_support: float=.60,
    top_per_degree: int=8, random_state: int=42
) -> StructuralReport:
    X=np.asarray(X,dtype=int); y=np.asarray(y,dtype=int)
    n=X.shape[1]
    if flag_names is None: flag_names=[f"F{i}" for i in range(n)]
    if len(flag_names)!=n: raise ValueError("flag_names no coincide con X")
    S=1-2*X

    skf=StratifiedKFold(folds,shuffle=True,random_state=random_state)
    baseline_pred=np.zeros(len(y)); fold_terms=[]
    candidate_count=0

    # Descubrimiento independiente en cada fold.
    ...
    # Frecuencia con que cada término aparece en el top-k de su grado.
    stats={}
    ...
    for term,scores in stats.items():
        ...
        # Validación incremental: añadir la interacción a un modelo base.
        deltas=[]
        for tr,te in skf.split(X,y):
            Sx=S
            Ftr=np.column_stack([X[tr][:,i] for i in range(n)] +
                                [np.prod(Sx[tr][:,term],axis=1)])
            Fte=np.column_stack([X[te][:,i] for i in range(n)] +
                                [np.prod(Sx[te][:,term],axis=1)])
        ...
        interpretation=("La señal aparece principalmente cuando "
                        "estas banderas actúan conjuntamente; "
                        "no debe interpretarse como causalidad.")
        hypotheses.append(StructuralHypothesis(
            names,len(term),coeff,coeff,support,delta,interpretation))
    hypotheses.sort(key=lambda h:(h.support_rate,h.mean_delta_auc,h.mean_abs_walsh),
                    reverse=True)

    # Modelo final usando las hipótesis estables.
    stable=[h.variables for h in hypotheses]
    name_to_idx={name:i for i,name in enumerate(flag_names)}
    idx_terms=[tuple(name_to_idx[v] for v in h.variables) for h in hypotheses]
    if idx_terms:
        cols=[X[:,i] for i in range(n)]
        cols += [np.prod(S[:,t],axis=1) for t in idx_terms]
    ...
    return StructuralReport(
        len(y),n,baseline_auc,enriched_auc,hypotheses,candidate_count)
