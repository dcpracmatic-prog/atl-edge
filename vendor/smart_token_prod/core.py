...
def _require_pq():
    ...
    if _pq is None:
        try:
            from pqcrypto.kem import ml_kem_768 as _pq_mod
...
# PBKDF2 work factor for offline dictionary resistance on salt/material verifiers.
# Override in tests via kdf_iterations=... (do not lower in production).
DEFAULT_KDF_ITERATIONS = 210_000
KDF_SALT_LEN = 16


def derive_coherence_secrets(
    master_secret: bytes,
    public_label: bytes,
    kdf_salt: bytes,
    iterations: int = DEFAULT_KDF_ITERATIONS,
) -> tuple:
    """Derive (salt, material) with a costly KDF so offline dictionary attacks
    against the verifiers stored in the .stok are expensive.

    Prior design used a single SHA-256 of master_secret (no public random salt,
    no work factor). Anyone with the .stok could test password candidates by
    hashing offline without ever calling open() / triggering friction.
    """
    if len(kdf_salt) < 8:
        raise ValueError("kdf_salt too short")
    if iterations < 1:
        raise ValueError("iterations must be positive")
    salt_input = kdf_salt + b"|STOK-KDF|" + public_label
    stretched = hashlib.pbkdf2_hmac(
        "sha256", master_secret, salt_input, int(iterations), dklen=32
    )
    salt = stretched[:16]
    material = hashlib.sha256(stretched + b"|MATERIAL|").digest()
    return salt, material
...
class SmartTokenProd:
    def __init__(
        ...
        # Coherence verifiers via PBKDF2 (offline dictionary resistance)
        self.kdf_iterations = int(kdf_iterations)
        self.kdf_salt = kdf_salt if kdf_salt is not None else os.urandom(KDF_SALT_LEN)
        self.salt, self.material = derive_coherence_secrets(
            master_secret, public_label, self.kdf_salt, self.kdf_iterations
        )
    ...
    def open(
        ...
        try:
            ...
            info["mlkem_decaps_ran"] = True
            # Implicit rejection: AES-GCM is the real authenticity gate
            info["mlkem_ok"] = bool(ss is not None)
