...
TEST_KDF_ITERATIONS = 2_000  # production default is 210_000; low for CI speed
...
def test_tampered_ciphertext_fails_aes_gcm():
    tok = SmartTokenProd(GOVERNANCE, b"payload-secreto", b"master", friction_backend="python", kdf_iterations=TEST_KDF_ITERATIONS)
...
def test_kdf_salt_present_and_offline_hash_not_enough(tmp_path):
    """salt/material require PBKDF2 with stored kdf_salt — not a single SHA-256."""
    from smart_token_prod.stok import protect_file, read_stok
    from smart_token_prod.core import derive_coherence_secrets

    sample = tmp_path / "a.bin"
    sample.write_bytes(b"payload-kdf")
    master = b"correct-horse-battery"
    stok_path, key_path = protect_file(
        sample,
        master_secret=master,
        tarpit_seconds=0.01,
    ...
    assert stok.kdf_salt and len(stok.kdf_salt) >= 8
    assert stok.kdf_iterations == TEST_KDF_ITERATIONS

    legacy = hashlib.sha256(master + b"|SALT|" + stok.public_label).digest()[:16]
    assert legacy != stok.salt

    salt, material = derive_coherence_secrets(
        master, stok.public_label, stok.kdf_salt, stok.kdf_iterations
    )
    assert salt == stok.salt and material == stok.material

    bad_salt, _ = derive_coherence_secrets(
        b"wrong-password", stok.public_label, stok.kdf_salt, stok.kdf_iterations
    )
    assert bad_salt != stok.salt


def test_native_python_working_key_parity_after_second_failure():
    """Parity of key-mutation path (2nd failure) between Python and C++ backends."""
    from smart_token_prod.core import SequentialTarpit
    from smart_token_prod.native import is_available
    from smart_token_prod.core import make_tarpit

    if not is_available():
        import pytest
        pytest.skip("native friction not built")

    key = bytes((i * 7 + 3) % 256 for i in range(32))
    py = SequentialTarpit(key, tarpit_mode="cpu", tarpit_seconds=0.0)
    nat = make_tarpit(key, "cpu", 0.0, "native")
    for _ in range(2):
        py.register_failure()
        nat.register_failure()
    ps, ns = py.snapshot(), nat.snapshot()
    assert ps["fail_count"] == ns["fail_count"] == 2
    assert ps["fib_seed"] == ns["fib_seed"]
    assert ps.get("flag_persistencia") is True and ns.get("flag_persistencia") is True
    pwk = py.state.working_key_material
    nwk = nat.current_key()
    assert len(pwk) == 32 and len(nwk) == 32
    assert pwk == nwk, (pwk[:8].hex(), nwk[:8].hex())
