...
def verify_entitlement(signed: SignedEntitlement, public_key_pem: bytes, *, now: Optional[float] = None,
    ...
    if organization_id and not hmac.compare_digest(ent.organization_id, organization_id):
        raise ValueError("organization mismatch")
    if node_id and ent.node_id and not hmac.compare_digest(ent.node_id, node_id):
