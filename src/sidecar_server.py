...
SIDECAR_SECRET = os.environ.get("ATL_SIDECAR_SECRET", "")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _verify_auth(self) -> bool:
        secret = getattr(self.server, "sidecar_secret", SIDECAR_SECRET)
        if not secret:
            return True
        ...
        elif "X-Sidecar-Token" in self.headers:
            token = self.headers.get("X-Sidecar-Token", "").strip()
        elif "X-Bearer-Token" in self.headers:
            token = self.headers.get("X-Bearer-Token", "").strip()
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser.add_argument("--secret", default=os.environ.get("ATL_SIDECAR_SECRET", ""))
    args = parser.parse_args(argv)

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    httpd = ThreadingHTTPServer((args.host, args.port), Handler)
    setattr(httpd, "sidecar_secret", args.secret)
    print(
        f"ATL sidecar listening on http://{args.host}:{args.port}  "
        f"available={is_available()}  data={DATA_DIR}  auth={'enabled' if args.secret else 'disabled'}",
        flush=True,
    )
    try:
        httpd.serve_forever()
