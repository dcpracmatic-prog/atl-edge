...
class StorageBackend(Protocol):
    ...
    def agent_count(self, license_id: str) -> int: ...

    def register_agent_atomic(self, license_id: str, agent_id: str, node_id: str,
                               instance_id: str, now: float, max_agents: int) -> None:
        """Register agent_id under license_id if capacity allows (atomic).

        Idempotent for the same agent_id. Raises StorageError if max_agents
        would be exceeded by a *new* agent identity.
        """
        ...
...
CREATE TABLE IF NOT EXISTS license_agents (
...
class SQLiteBackend:
    ...
    def agent_count(self, license_id: str) -> int:
        ...
        try:
            row = conn.execute(
                "SELECT COUNT(*) AS n FROM license_agents WHERE license_id = ?",
                (license_id,),
            ).fetchone()
    ...
    def register_agent_atomic(self, license_id: str, agent_id: str, node_id: str,
        ...
        try:
            conn.execute("BEGIN IMMEDIATE")
            exists = conn.execute(
                "SELECT 1 FROM license_agents WHERE license_id = ? AND agent_id = ?",
                (license_id, agent_id),
            ).fetchone()
            if exists:
                conn.execute(
                    "UPDATE license_agents SET node_id = ?, instance_id = ?, registered_at = ? "
                    "WHERE license_id = ? AND agent_id = ?",
                    (node_id, instance_id, now, license_id, agent_id),
                )
            else:
                n = conn.execute(
                    "SELECT COUNT(*) AS n FROM license_agents WHERE license_id = ?",
                ...
                if int(n) >= int(max_agents):
                    raise StorageError("max_agents exceeded")
                conn.execute(
                    "INSERT INTO license_agents (license_id, agent_id, node_id, instance_id, registered_at) "
                    "VALUES (?, ?, ?, ?, ?)",
                    (license_id, agent_id, node_id, instance_id, now),
                )
            conn.execute("COMMIT")
        except StorageError:
            raise
...
class PostgresBackend:
    ...
    def agent_count(self, license_id: str) -> int:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) FROM license_agents WHERE license_id = %s",
                (license_id,),
            )
            row = cur.fetchone()
            return int(row[0]) if row else 0
    ...
    def register_agent_atomic(self, license_id: str, agent_id: str, node_id: str,
        ...
        with self._conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    "SELECT 1 FROM license_agents WHERE license_id = %s AND agent_id = %s FOR UPDATE",
                    (license_id, agent_id),
                )
                exists = cur.fetchone()
                if exists:
                    cur.execute(
                        "UPDATE license_agents SET node_id = %s, instance_id = %s, registered_at = %s "
                        "WHERE license_id = %s AND agent_id = %s",
                        (node_id, instance_id, now, license_id, agent_id),
                    )
                else:
                    cur.execute(
