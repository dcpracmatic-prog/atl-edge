...
def main() -> int:
    ...
    iss = sub.add_parser(
        "issue-license",
        help="Admin: create a license in a local control-plane state dir; stdout shows fingerprint only",
    )
    ...
    gen = sub.add_parser(
        "generate-api-key",
        help="development-only random key (not issued by control plane)",
    )
    gen.add_argument(
        "--print-api-key",
        action="store_true",
        help="print raw key to stdout (also allowed via ATL_PRINT_API_KEY=1); default writes 0600 once-file only",
    )
    gen.add_argument(
        "--out",
        type=Path,
        default=Path(".atl") / "api_key.once.txt",
        help="path for mode-0600 once-file when not printing",
    )
    ...
    if args.cmd == "generate-api-key":
        ...
        allow_print = bool(args.print_api_key) or os.environ.get("ATL_PRINT_API_KEY") == "1"
        if allow_print:
            # Explicit operator opt-in only (flag or ATL_PRINT_API_KEY=1).
            print(key)
            return 0
        key_path = _write_api_key_once(args.out, key)
        print(
            json.dumps(
                {
                    ...
                    "note": "Raw key not printed; pass --print-api-key or ATL_PRINT_API_KEY=1 to print once.",
                },
                indent=2,
            )
        )
